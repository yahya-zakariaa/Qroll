...
export default function Lectureattendstudent() {
  const navigate = useNavigate();
  const { id } = useParams();
  const { getLecturesAttendace } = useStudentStore();
  ...
  const fetchLecture = async () => {
    try {
      setIsLoading(true);
      const res = await getLecturesAttendace(id);
      setLectures(res);
    } catch (error) {
  ...
  return (
    <div>
      <div className="flex gap-6 md:m-10 max-md:m-3 items-center">
        ...
        <i className="fa-solid fa-chevron-right" style={{ color: "#71717a" }} />
        <h1 className="text-[#71717A] "> LECTURE ATTENDENCE REPORT </h1>
      </div>
        ...
        {lectures?.length > 0 ? (
          <>
             <table className="w-full text-sm text-left ">
                <thead className="">
                  <tr>
                    <th className="p-2 text-[#A1A1AA] ">lecture number</th>
                    <th className="p-2 text-[#A1A1AA] "> lecture name</th>
                ...
                <tbody>
                  {currentLectures.map((item) => (
                    <tr key={item.id} className="border-t">
                      <td className="p-2 ">{item?.lecture?.id}</td>
                      <td className="p-2 ">
                        {item?.lecture?.name}
                      </td>
                      <td className="p-2 ">
                        {new Date(item?.lecture?.created_at).toLocaleDateString()}
                      </td>
