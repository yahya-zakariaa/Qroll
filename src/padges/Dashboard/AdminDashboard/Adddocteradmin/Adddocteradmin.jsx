...
export default function Adddocteradmin() {
  ...
  const { getDoctors, deleteDoctor } = useAdminStore();
  const pageSize = 9;
  ...
  const fetchDoctors = async () => {
    try {
      ...
      setUsers(res);
      setFilteredUsers(res); // تعيين التصفية المبدئية
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const handleDeleteDoctor = async (id) => {
    ...
    try {
      const res = await toast.promise(deleteDoctor(id), {
        ...
        <div className="header flex md:flex-row flex-col-reverse items-center justify-between">
          ...
          <div className="flex ">
            ...
            <button className=" max-md:text-[13px] flex items-center self-center justify-center bg-[#161B39] text-[white] m-2 w-48  rounded-[8px] h-11">
              ...
              <p
                className="p-2"
                onClick={() => navigate("/admin-dashboard/newdocteradmin")}
              >
                Add new doctor{" "}
              </p>
            </button>
                    ...
                    <td className="px-4 py-2 text-sm text-gray-600">
                      <div className="flex gap-2">
                        <button
                          onClick={() =>
                            navigate(
                              `/admin-dashboard/veiwdocteradmin/${item.doctor_id}`
                            )
                          }
                        ...
                        <button
                          onClick={() => handleDeleteDoctor(item?.doctor_id)}
